...
static struct kernelmap {
  void* virtualmem;
  uint64_t phys_start;
  uint64_t phys_end;
  int permission;
} kernelmap[] = {
  { (void*) KERNBASE, 0, PHYSEND, 0 }
};
...
int setupkvm( void ) {
  ...
  void* va = (void*) ALIGNDN( (uint64_t)kernelmap->virtualmem );
  void* pa = kernelmap->phys_start;
  ...
  for( ;; ) {
	...
	return 0;
      ...
      *pml4e = (uint64_t) V2P( pdpt ) | PTE_P | PTE_W | PTE_U;
    }
